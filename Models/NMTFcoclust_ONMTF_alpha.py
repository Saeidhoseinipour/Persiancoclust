...
class ONMTF:
	...
	def _fit_single(self, X, random_state, y=None) :
		...
		while change :
			...
			S = DF@S@DG
			G = (np.diagflat(np.power(G.sum(axis = 0), -1))@G.T).T 
			...
			Orthogonal_F_list.append(linalg.norm(F.T@F - I_g, 'fro'))
			Orthogonal_G_list.append(linalg.norm(G.T@G - I_s, 'fro'))
			...
			c = c_0 * np.sum(np.multiply(X,f_z)) + self.delta* np.trace(F@P_g@F.T) + self.beta* np.trace(G@P_s@G.T)
